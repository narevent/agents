...
echo "================================"
echo "agents Django Deployment"
echo "================================"

# Configuration
PROJECT_DIR="/var/www/agents"
REPO_URL="https://github.com/narevent/agents.git"
...
if [ ! -f "$PROJECT_DIR/.env" ]; then
    ...
    echo ""
    echo "IMPORTANT: Edit /var/www/agents/.env and configure:"
...
sudo cp $PROJECT_DIR/nginx.conf /etc/nginx/sites-available/agents
sudo ln -sf /etc/nginx/sites-available/agents /etc/nginx/sites-enabled/
...
if [ "$setup_ssl" = "y" ]; then
    ...
    sudo certbot --nginx -d agents.vetgaaf.tech --non-interactive --agree-tos --email $email --redirect
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "sudo certbot --nginx -d agents.vetgaaf.tech"
    fi
else
...
if [ "$setup_ssl" = "y" ]; then
    echo "  https://agents.vetgaaf.tech"
    echo "  Admin panel: https://agents.vetgaaf.tech/admin/"
else
    echo "  http://example.com (HTTP only - setup SSL for HTTPS)"
    echo "  Admin panel: http://agents.vetgaaf.tech/admin/"
fi
echo ""
